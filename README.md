...
一个仅供个人使用的轻量求职邮件自动处理工具。每天北京时间 07:50 从 QQ 邮箱读取最近 7 天邮件，将新增测评、笔试和面试事项写入飞书多维表格、同步到 iCloud Calendar，并在 08:00 前发送客观早报。
...
- 每天北京时间 `07:50` 运行，并显式使用 `Asia/Shanghai` 时区；避开 GitHub Actions 整点拥堵，通常在 08:00 前完成并发送早报。
