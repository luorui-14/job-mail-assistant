...
一个仅供个人使用的轻量求职邮件自动处理工具。每天北京时间 00:37 起由 GitHub Actions 调度，通常从 QQ 邮箱读取最近 2 天邮件；若上次成功运行更早，则从上次成功运行时间起回扫。带明确入口和截止时间的岗位投递，以及新增测评、笔试和面试事项会写入飞书多维表格、同步到 iCloud Calendar，并尽量在 08:00 前发送客观早报。GitHub 的定时任务可能排队延迟，因此预留了整个后半夜的交付余量。
...
AI 只理解自然语言和提取时间组成部分。小时、自然日、中国工作日、明确日期及星期表达均由 Python 计算；无法可靠确定的时间留空并进入“需要人工确认”。岗位是可选信息：邮件未提供时保持为空，不会因此进入人工确认，报告中也不显示“岗位待确认”。邮件接收基准使用 IMAP `INTERNALDATE`，不使用邮件头 `Date` 或任务执行时间。

测评、笔试或异步 AI 面试邀请同时写有生效与失效时刻时，以唯一且明确标注“失效”的时刻作为截止时间。若已处理记录漏填或误填该时间，邮件仍在正常扫描窗口内时会重新处理并更新原记录。

岗位发布邮件只有同时具备招聘语境、投递/申请动作和明确截止表达时才会进入处理，普通岗位推广不会创建事项。HTML 邮件的纯文本部分若只是简短占位，程序会使用信息更完整的 HTML 渲染文本，避免遗漏其中的截止日期。

真人视频、电话或现场面试只有在邮件已经明确确认实际面试日期和时间时才会写入 Base 和
Calendar。邀请预约/选择/协商面试时间、只给出预约截止时间，以及面试后的问卷、满意度或
体验反馈都不属于面试安排，会被排除。异步 AI 面试、测评和笔试仍按完成截止时间跟踪。
...
- 单选字段 `类型` 需包含 `投递`、`测评`、`笔试`、`AI面试` 及各面试阶段选项。

- 文本：`Message-ID`、`邮件指纹`、`原邮件主题`、`原始时间描述`、`确认说明`、`Calendar Event ID`、`Calendar 状态`、`Calendar 错误`、`时间类型`
- 复选框：`时间为推算`、`需要人工确认`
- 日期时间：`最后处理时间`、`结束时间`

QQ 网页版没有可由 IMAP `UID` 或 `Message-ID` 稳定生成的单封邮件链接，
因此程序不写入仅能打开邮箱首页的误导性链接。可使用同一行的 `原邮件主题`、
`邮件接收时间` 和 `Message-ID` 精确核查来源。
...
- `References`/`In-Reply-To` 命中已有记录时更新原事项；岗位为空时，同公司、同类型、同一
  时间的提醒邮件也会合并到已有事项。
...
测试覆盖时间计算、中国节假日和调休、岗位投递截止与富 HTML 邮件过滤、链接约束、Message-ID/线程去重、24 小时提醒，以及连续运行两次不重复写表或建日历。
